...
from app.auth.security import HMACVerificationError, verify_hmac_signature
from app.database.session import get_db
from app.logic.ai_backend_reliability import (
    AIBackendReliabilityManager,
    RoutedDebateResult,
)
from app.logic.ai_council import DebateResult
from app.logic.budget_integration import TradeGatingContext, check_trade_allowed
from app.logic.operational_gating import GatingSignal
from app.logic.risk_manager import RiskProfile, calculate_position_size
from app.observability.discord_notifier import AlertLevel, DiscordNotifier, EmbedColor
from app.observability.metrics import record_signal_executed, record_signal_received
...
async def receive_tradingview_signal(
    ...
    try:
        verify_hmac_signature(
            payload=raw_body, provided_signature=x_tradingview_signature
        )
        hmac_verified = True
    except HMACVerificationError as e:
        print(f"[{e.error_code}] HMAC verification failed: {e.message}")
        return create_error_response(
            error_code=e.error_code, message=e.message, status_code=401
        )
    ...
    try:
        ...
        if "AUD-001" in error_msg:
            return create_error_response(
                error_code="AUD-001", message=error_msg, status_code=422
            )
        raise
    ...
    try:
        ...
        result = db.execute(
            insert_sql,
            {
                ...
                if hasattr(signal_in.side, "value")
                else str(signal_in.side),
        ...
        if (
            "signals_signal_id_unique" in error_str
        ...
        if "signals_side_check" in error_str or "check constraint" in error_str.lower():
            return create_error_response(
                error_code="VAL-003",
                message="Invalid side value. Must be 'BUY' or 'SELL'.",
                status_code=422,
            )
    ...
    try:
        ...
        risk_profile = calculate_position_size(
            signal_price=signal_in.price, correlation_id=str(correlation_id)
        )
    ...
    try:
        ...
        db.execute(
            risk_insert_sql,
            {
                ...
                "risk_percentage": str(risk_profile.risk_percentage)
                if risk_profile
                else "0.01",
                "risk_amount_zar": str(risk_profile.risk_amount_zar)
                if risk_profile
                else "0",
                "calculated_quantity": str(risk_profile.calculated_quantity)
                if risk_profile
                else "0",
                "status": risk_status,
    ...
    # ========================================================================
    # STEP 10: COLD PATH AI - AI Council Debate via Reliability Manager
    # ========================================================================
    # Only proceed with AI debate if budget AND risk assessment were approved.
    # All provider routing goes through AIBackendReliabilityManager —
    # no ad-hoc provider selection (Phase 7 mandate).
    ai_consensus = "SKIPPED"
    ai_rejection_reason: Optional[str] = None
    debate_result: Optional[DebateResult] = None
    routed_result: Optional[RoutedDebateResult] = None
    ...
    if (
        ...
        try:
            # Route debate through reliability manager (Phase 7)
            ...
            if routed_result.final_verdict:
                ai_consensus = "APPROVED"
            else:
                ai_consensus = "REJECTED"
                ai_rejection_reason = routed_result.reject_reason or (
                    f"Consensus score "
                    f"{debate_result.consensus_score if debate_result else 0}/100 "
                    f"(requires unanimous approval)"
                )

            print(
                f"[AI-COUNCIL] correlation_id={correlation_id} | "
                f"provider={routed_result.provider_used.value if routed_result.provider_used else 'NONE'} | "
                f"consensus={debate_result.consensus_score if debate_result else 0} | "
                f"verdict={ai_consensus} | "
                f"routing={routed_result.routing_decision.reason}"
            )

        except Exception as e:
            # AI Council error - default to REJECTED (safety first)
            ai_consensus = "REJECTED"
            ai_rejection_reason = f"AI Reliability Manager error: {str(e)[:200]}"
            print(f"[AI-ERR] Reliability manager failed for {correlation_id}: {e}")
    ...
    response = {
        ...
        "budget_gating": {
            ...
            "budget_correlation_id": budget_gating.budget_correlation_id
            if budget_gating
            else None,
            "gating_signal": budget_gating.gating_signal.value
            if budget_gating
            else None,
            "risk_level": budget_gating.risk_level.value
            if budget_gating and budget_gating.risk_level
            else None,
            "operational_cost_zar": str(budget_gating.operational_cost_zar)
            if budget_gating and budget_gating.operational_cost_zar
            else None,
            "net_alpha_zar": str(budget_gating.net_alpha_zar)
            if budget_gating and budget_gating.net_alpha_zar
            else None,
        ...
        "trade_decision": {"status": final_status, "action": trade_action},
    }
